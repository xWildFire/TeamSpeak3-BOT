...
class Instance extends Thread {
	...
	function run(){
		...
		$cmds = array();
		$loaded = array();
		foreach(glob($this->config['settings']['path'].'/*.php') as $file){
			$class = str_replace('.php', '', basename($file));
			if(in_array($class, $this->config['disabled']))
				continue;
			include($file);
		...
		$this->loaded = (array)$loaded;
		$this->cmds = (array)$cmds;
		
		if(@$this->config['settings']['commands']){
			while(true){
				$data = '';
				do{
					$this->version();
					$this->selectServer($this->connection['server_port']);
					$whoami = $this->whoAmI();
					$name = 'wBOT @ '.$this->connection['bot_name'];
					if($whoami['client_nickname'] != $name){
						$changed = $this->setName($name);
						$i = 0;
						while(@$changed['id'] == 513)
							$changed = $this->setName($name."(".++$i.")");
					}
				...
				} while(strpos($data, 'notifytextmessage targetmode=1 msg=') === false && strpos($data, 'notifytextmessage targetmode=2 msg=') === false);

				$this->doCommand($data, $instances);
			}
		}else{
			while(true){
				if(!$this->isConnected()){
					sleep(1);
					continue;
				}else if(filemtime('config/config.json') > $this->last){
					$config = loadConfig();
					if(json_last_error() == 0){
						$this->setConfig($config[$this->id]);
						$this->last = time();
					}
				}
				$this->selectServer($this->connection['server_port']);
				...
				$channels = array();
				foreach($this->channelList('-topic -flags') as $channel)
					$channels[$channel['cid']] = $channel;
				$clients = array();
				foreach($this->clientList('-away -voice -times -groups -ip -uid -info') as $client){
				...
				$time = time();
				$loaded = $this->loaded;
				foreach($this->loaded as $func => $last){
					if($last > $time)
						continue;
					
					$func::execute($this, $clients, $channels, $server);
					$loaded[$func] = $time+@$this->config['functions'][$func]['execute'];
				}
				$this->loaded = (array)$loaded;
				
				clearstatcache();
				sleep($this->config['settings']['idle'] > 0 ? $this->config['settings']['idle'] : 0);
			}
		}
	}
